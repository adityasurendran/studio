...
if (!process.env.GOOGLE_API_KEY || process.env.GOOGLE_API_KEY === "YOUR_GOOGLE_AI_API_KEY") {
  const message = !process.env.GOOGLE_API_KEY
    ? "nyro Genkit Setup: GOOGLE_API_KEY is not set in your environment variables. "
    : "nyro Genkit Setup: GOOGLE_API_KEY appears to be the placeholder 'YOUR_GOOGLE_AI_API_KEY'. ";
